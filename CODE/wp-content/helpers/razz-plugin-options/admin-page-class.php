 ...
 */
  ...
  class BF_Admin_Page_Class {
    ...
    public function __construct($args) {
	  ...
	  $this->SelfPath = '/../wp-content/helpers/razz-plugin-options';
	  /*
	...
	public function getFontCSS($typo, $important=false)
	{
		if(!$typo || !is_array($typo)) return "";
		$imp = ($important) ? " !important" : "";
		$font = $this->get_fonts_family($typo['face']);
		$css = $font['css'];
		$css .= "font-size: {$typo['size']};";
		
		$variation = $typo['variation'];
		$variation = str_replace('regular', 'normal', $variation);
		
		if(preg_match('/^[^0-9]/', $variation))
		{
			$weight = '400';
			$style = $variation;
		}
		elseif(preg_match('/^[0-9][0-9][0-9].+/', $variation))
		{
			$weight = substr($variation, 0, 3);
			$style = substr($variation, 3);
		}
		else
		{
			$weight = $variation;
		...
		$css .= "font-weight: {$weight}{$imp};";
		$css .= "font-style: {$style}{$imp};";
		$css .= "color: {$typo['color']}{$imp};";
		return $css;
	}
	
	public function enqueue_fonts()
	{
		$data = get_option($this->option_group);
		$loadfonts = array();
		foreach ($this->_fields as $field)
		{
			if($field['type'] != 'typo') continue;
			if(!($value = $data[$field['id']])) continue;
			$font = $this->get_fonts_family($value['face']);
			if(!($import = $font['import'])) continue;
			
			if(!array_key_exists($import, $loadfonts))
			{
				$loadfonts[$import] = array();
			}
			if(!array_key_exists($value['variation'], $loadfonts[$import]))
			{
				$loadfonts[$import][] = $value['variation'];
			}
		}
		$families = array();
		foreach($loadfonts as $import => $variations)
		{
			$families[] = $import . ':' . implode(',', $variations);
		}
		$family = implode('|', $families);
		
		if(!empty($family))
		{
			wp_enqueue_style("{$this->option_group}_gfonts", "http://fonts.googleapis.com/css?family={$family}");
		}
	}
    ...
    */
     ...
     * @access private
     */
    private function SetField($args, $repeater=false) {
      $default = array(
          'std' => '',
          'id' => ''
       );
    ...
    public function Subtitle($label, $repeater=false) {
      ...
      return $this->SetField($args, $repeater);
    }
  ...
  */
  ...
  public function check_field_color() {
    ...
    if ( $this->has_field_any(array('color' ,'typo' ))  && $this->is_edit_page() ) {
      ...
      wp_enqueue_style('spectrum', $plugin_path.'/css/spectrum.css' );
      wp_enqueue_script('spectrum', $plugin_path . '/js/spectrum.js', array( 'jquery'), null, true );
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_radio( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      foreach ( $field['options'] as $key => $value ) {
        echo "<input type='radio' class='at-radio".(isset($field['class'])? " {$field['class']}": "")."' id='{$field['id']}_{$key}' name='{$field['id']}' value='{$key}'" . checked( in_array( $key, $meta ), true, false ) . " /> <label for='{$field['id']}_{$key}' style='display: initial;'><span class='at-radio-label'>{$value}</span></label>";
      }
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_typo( $field, $meta ) {
    ...
    foreach ( $faces as $key => $face ) {
      $op .= '<option data-variants="' . esc_attr(json_encode($face['variants'])) . '" value="' . esc_attr( $key ) . '">' . esc_html( $face['name'] ) . '</option>';
    }
    if (isset($meta['face']))
    ...
    // Font Variation
    $html .= '<select class="at-typography at-typography-variation" name="' . esc_attr( $field['id'] .'[variation]' ) . '" id="' . esc_attr( $field['id'] . '_variation' ) . '" style="width: 100px;">';
    //$weights = $this->get_font_variations($faces[$meta['face']]['variants']);
    $op = '';
    foreach($faces[$meta['face']]['variants'] as $key => $label) {
      $op .= '<option value="' . esc_attr( $key ) . '">' . esc_html( $label ) . '</option>';
    }
    if (isset($meta['variation']))
      $op = str_replace('value="'.$meta['variation'].'"', 'value="'.$meta['variation'].'" selected="selected"', $op);
    $html .= $op. '</select>';
    ...
    // Font Color
	$html .= "<input class='at-color-picker' type='text' name='{$field['id']}[color]' id='{$field['id']}[color]' value='{$meta['color']}'  />";
  ...
  public function show_field_color( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
	echo "<input class='at-color-picker".(isset($field['class'])? " {$field['class']}": "")."' type='text' name='{$field['id']}' id='{$field['id']}' value='{$meta}'  />";
    $this->show_field_end($field, $meta);
  ...
  public function save($repeater = false) {
	...
	do_action('admin_page_class_after_save');
  }
  ...
  public function get_fonts_family($font = null) {
    $fonts = get_option('WP_EX_FONTS_LIST5', $default = false);
    if ($fonts === false){
      $fonts = array(
          'arial' => array(
            'name' => 'Arial',
			'variants' => $this->get_font_variations(),
      ...
      if ($this->google_fonts){
        $gs = wp_remote_get( 'https://www.googleapis.com/webfonts/v1/webfonts?sort=popularity&key=AIzaSyAxE6bU6VCtqLrRtISuQL9FCBi2Pfpp2m4', array('sslverify' => false)); 
        if(! is_wp_error( $gs ) ) {
          ...
          foreach ( $fontArray as $f ){
			...
			{
				$variants[$variant] = $this->prettyFontVariant($variant);
			}
            $fonts[$key] = array(
              'name' => $f->family,
	  ...
	  ksort($fonts);
      update_option('WP_EX_FONTS_LIST5',$fonts);
    }
   ...
   * @return array
   */
  public function get_font_variations(){
    $default = array(
      'normal' => 'Normal',
      'bold' => 'Bold',
      'bolder' => 'Bolder',
  ...
  function prettyFontVariant($font)
  {
	if(preg_match('/^[0-9][0-9][0-9].+/', $font))
	{
		$font = substr($font, 0, 3) . ' ' . substr($font, 3);
	}
	return ucwords($font);
  }
