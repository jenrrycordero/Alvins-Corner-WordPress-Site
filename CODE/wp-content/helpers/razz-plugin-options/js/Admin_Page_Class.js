...
function loadColorPicker(){
	if($('.at-color-picker').length>0){
		$('.at-color-picker').spectrum({
			showInput: true,
			showInitial: true,
			showAlpha: true,
			...
			palette: [],
			localStorageKey: "razz.colors"
		});
	}
	
	if($('select.at-typography-face').length>0){
		$('select.at-typography-face').on('change', function()
		{
			
			var $selected = $(this).find(':selected');
			var $field = $(this).closest('.field');
			var $variant = $field.find('select.at-typography-variation');
			var variants = $selected.data('variants');
			
			$variant.html('');
			for(var k in variants)
			{
				$variant.append('<option value="' + k + '">' + variants[k] + '</option>')
			}
			$variant.val('regular');
			$field.find('div.at-typography-variation span').html('Regular');
		});
	}
}
